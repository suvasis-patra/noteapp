...
import { ZLoginUser, ZRegisterUser } from "../schemas/user.schema";

export const registerUser = asyncHandler(
  async (req: Request, res: Response) => {
    // validate user details
    const userInfo = ZRegisterUser.safeParse(req.body);
    if (!userInfo.success) {
    ...
    res
      .status(201)
      .json(new ApiResponse<IUserRegistration>(201, user, "registered"));
  }
);

export const loginUser = asyncHandler(async (req: Request, res: Response) => {
  // validate user information
  const userInfo = ZLoginUser.safeParse(req.body);
  if (!userInfo.success) {
    throw new ApiError(400, "Invalid user details");
  }
